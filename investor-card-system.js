...
const InvestorCardSystem = (function() {
   ...
   function addSidebarButtons() {
    ...
    if (document.querySelector('.menu-item[href="#investor-cards"]')) {
        return;
    }
    ...
    if (!sidebarMenu) {
        console.error('لم يتم العثور على قائمة الشريط الجانبي');
        return;
    }
    ...
    menuItem.innerHTML = `
        <a href="#investor-cards" class="menu-item">
            <div class="menu-icon">
    ...
    console.log('تم إضافة بطاقات المستثمرين إلى القائمة الجانبية');
}
...
function initScannerAfterLoad() {
    ...
    if (cardsPage) {
        ...
        const cardMenuItem = document.querySelector('.menu-item[href="#investor-cards"]');
        if (cardMenuItem) {
            cardMenuItem.classList.add('active');
        }
    }
}
...
if (typeof InvestorCardSystem !== 'undefined') {
    InvestorCardSystem.initialize()
        .then(success => {
            console.log('تم تهيئة نظام بطاقات المستثمرين:', success);
            investorCardSystem = InvestorCardSystem;
        })
        .catch(error => {
            console.error('خطأ في تهيئة نظام بطاقات المستثمرين:', error);
        });
...
// تحديث دالة printCard للتوافق مع نظام الطباعة الرئيسي
function printCard(cardId) {
    // ... كود الطباعة الموجود ...
    
    // إضافة إحصائية الطباعة إذا كان النظام يدعمها
    if (window.createActivity) {
        window.createActivity('card', 'print', `طباعة بطاقة ${card.investorName}`);
    }
}

// تحديث دالة createCard لإضافة إشعار
function createCard(investorId, cardType, expiryDate, options = {}) {
    // ... كود إنشاء البطاقة ...
    
    // إضافة إشعار بإنشاء البطاقة
    if (window.createNotification) {
        window.createNotification(
            'بطاقة جديدة',
            `تم إنشاء بطاقة ${cardType} للمستثمر ${investor.name}`,
            'success',
            newCard.id,
            'card'
        );
...
function saveData() {
    try {
        ...
        // حفظ بيانات البطاقات إذا كان النظام موجوداً
        if (window.InvestorCardSystem && window.InvestorCardSystem.cards) {
            localStorage.setItem('investor_cards', JSON.stringify(window.InvestorCardSystem.cards));
        }
